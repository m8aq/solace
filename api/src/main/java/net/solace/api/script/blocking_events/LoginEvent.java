...
extends BlockingEvent {
    ...
    public LoginEvent(BlockingEventManager blockingEventManager) {
        this(blockingEventManager, () -> true);
    }

    @Override
    public boolean validate() {
        return !Game.isLoggedIn() && Game.getState() != GameState.HOPPING && (Game.getGameAccount() != null || Client.isOAuthCredentialsSet());
    }

    @Override
    public int loop() {
        String loginMessage = Client.getLoginMessage();
        ...
        return GameThread.invokeAndWait(this::login);
    }

    public int login() {
        int loginState = Client.getWrapped().getLoginIndex();
        GameAccount gameAccount = Game.getGameAccount();
        log.debug("Login state: {}", (Object)loginState);
        if (gameAccount == null && !Client.isOAuthCredentialsSet()) {
            log.warn("No account set, stopping login event.");
            return -1000;
        }
        if (this.attemptedLogins > 0) {
            if (this.attemptedLogins >= 5) {
                log.warn("Attempted to log in multiple times, stopping.");
                return -1000;
            }
            if (loginState == 3) {
                this.prepareLogin();
        ...
        switch (loginState) {
            case 4: {
                log.warn("Account requires an authenticator code; two-factor login is not supported.");
                return -1000;
            }
            case 14: {
            ...
            case 34: {
                log.warn("Account requires membership.");
                return -1000;
            }
            case 3: {
                log.warn("Invalid account set.");
                ++this.attemptedLogins;
                return 5000;
            }
            case 10: {
                this.jlLogin();
        ...
        this.prepareLogin();
        if (gameAccount != null) {
            this.enterCredentials(gameAccount);
        } else if (!Client.isOAuthCredentialsSet()) {
            log.error("No account set, and not using JL, cannot enter credentials.");
            return -1000;
        }
        return 2000;
    }
    ...
    private void enterCredentials(GameAccount account) {
    ...
    public LoginEvent(BlockingEventManager blockingEventManager, Supplier<Boolean> condition) {
