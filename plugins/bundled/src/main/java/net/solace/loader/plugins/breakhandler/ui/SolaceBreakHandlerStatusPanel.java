...
@Slf4j
public class SolaceBreakHandlerStatusPanel extends JPanel {
    ...
    SolaceBreakHandlerStatusPanel(SolaceBreakHandlerPlugin solaceBreakHandlerPlugin, BreakHandler solaceBreakHandler, Plugin plugin) {
        ...
        if (solaceBreakHandlerPlugin.disposables.containsKey(plugin)) {
            Disposable seconds = solaceBreakHandlerPlugin.disposables.get(plugin);
        ...
        solaceBreakHandlerPlugin.disposables.put(plugin, secondsDisposable);

        Disposable extraDataDisposable = solaceBreakHandler
                .getExtraDataObservable()
                .subscribe((data) -> syncExec(() -> this.extraData(data)));

        solaceBreakHandlerPlugin.disposables.put(plugin, extraDataDisposable);
    ...
    private void milliseconds(long ignored) {
        ...
        Map<Plugin, Instant> startTimes = solaceBreakHandler.getStartTimes();

        if (startTimes.containsKey(plugin)) {
            Duration duration = Duration.between(solaceBreakHandler.getStartTimes().get(plugin), now);
            runtimeLabel.setText(formatDuration(duration));
        }

        Map<Plugin, Integer> breaks = solaceBreakHandler.getAmountOfBreaks();

        if (breaks.containsKey(plugin)) {
            breaksLabel.setText(String.valueOf(solaceBreakHandler.getAmountOfBreaks().get(plugin)));
        }

        if (!solaceBreakHandler.isBreakPlanned(plugin) && !solaceBreakHandler.isBreakActive(plugin)) {
            timeLabel.setText("00:00:00");
            return;
        }

        if (solaceBreakHandler.isBreakPlanned(plugin)) {
            Instant breaktime = solaceBreakHandler.getPlannedBreak(plugin);
        ...
        } else if (solaceBreakHandler.isBreakActive(plugin)) {
            Instant breaktime = solaceBreakHandler.getActiveBreak(plugin);
        ...
        boolean enabled = Boolean.parseBoolean(configManager.getConfiguration(CONFIG_GROUP, sanitizedName(plugin) + "-enabled"));

        if (enabled && solaceBreakHandler.getPlugins().get(plugin) && solaceBreakHandlerPlugin.isValidBreak(plugin) && !solaceBreakHandler.isBreakPlanned(plugin) && !solaceBreakHandler.isBreakActive(plugin)) {
            solaceBreakHandlerPlugin.scheduleBreak(plugin);
        }
    }
    ...
    private void extraData(Map<Plugin, Map<String, String>> extraData) {
        ...
        extraDataPanel.setBackground(SolaceBreakHandlerPanel.BACKGROUND_COLOR);
    ...
    private void onConfigChanged(ConfigChanged configChanged) {
        ...
        if (configChanged.getKey().contains("enabled") && configChanged.getNewValue().equals("false")) {
            solaceBreakHandler.removePlannedBreak(plugin);
        }
    ...
    private void init() {
        ...
        status.setFont(SolaceBreakHandlerPanel.SMALL_FONT.deriveFont(16f));
    ...
    private void statusPanel() {
        ...
        if (!solaceBreakHandler.getPlugins().get(plugin) && !solaceBreakHandler.isBreakActive(plugin)) {
            c.insets = new Insets(2, 0, 2, 0);
            c.weightx = 0;
            c.gridx = 0;
            c.gridy = 0;
        ...
        if (solaceBreakHandler.getPlugins().get(plugin)) {
            boolean enabled = Boolean.parseBoolean(configManager.getConfiguration(CONFIG_GROUP, sanitizedName(plugin) + "-enabled"));
            ...
            if (solaceBreakHandler.getPlugins().get(plugin) && solaceBreakHandlerPlugin.isValidBreak(plugin) && !solaceBreakHandler.isBreakPlanned(plugin) && !solaceBreakHandler.isBreakActive(plugin)) {
                solaceBreakHandlerPlugin.scheduleBreak(plugin);
            }
        }
    ...
    private void infoPanel() {
        ...
        infoPanel.setBackground(SolaceBreakHandlerPanel.BACKGROUND_COLOR);
