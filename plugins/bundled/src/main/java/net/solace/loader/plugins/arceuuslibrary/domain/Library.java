...
public class Library {
    private final List<LibraryEventListener> libraryEventListeners = new CopyOnWriteArrayList<>();
    ...
    private final Map<Integer, ArrayList<Bookcase>> byRoom = new HashMap<>();
    @Getter
    private final Map<Book, Bookcase> byBook = new HashMap<>();
    ...
    @Getter
    private boolean groundFloorChecked;
    @Getter
    @Setter
    ...
    public void register(LibraryEventListener listener) {
        synchronized (this.mutex) {
            this.libraryEventListeners.add(listener);
        }
    }

    public void clearListeners() {
        synchronized (this.mutex) {
            this.libraryEventListeners.clear();
        }
    }

    public void notify(LibraryEvent event) {
        for (LibraryEventListener listener : libraryEventListeners) {
            listener.notify(event);
        }
    }

    public void setGroundFloorChecked() {
        groundFloorChecked = true;
    }

    public synchronized List<Bookcase> getBookshelvesOnLevel(int z) {
        return Collections.unmodifiableList(byLevel.get(z));
    }

    public synchronized List<Bookcase> getBookshelvesByRoom(int room) {
        return Collections.unmodifiableList(byRoom.get(room));
    }

    public synchronized Bookcase getBookshelfByBook(Book book) {
        return byBook.get(book);
    }

    public synchronized List<Bookcase> getBookshelves() {
        return Collections.unmodifiableList(byIndex);
    }

    public void setCustomer(Customer customer, Book book) {
    ...
    public synchronized void reset() {
        ...
        byBook.clear();
        groundFloorChecked = false;
        MessageUtils.addMessage("Resetting library state");
        notify(new LibraryEvent("reset"));
        shouldHop = true;
    }

    public void mark(int bookcaseIndex, Book book) {
        mark(byIndex.get(bookcaseIndex), book);
    }

    public void mark(WorldPoint loc, Book book) {
    ...
    public synchronized void mark(Bookcase bookcase, Book book) {
        if (bookcase.isBookSet()) {
            ...
            if (book != bookcase.getBook()
                && !(book == null && bookcase.getBook() == Book.VARLAMORE_ENVOY)) {
                reset();
            }
        ...
        if (state == SolvedState.COMPLETE) {
            ...
            if (book == null
                ...
                && bookcase.getPossibleBooks().stream().noneMatch(b -> b == Book.VARLAMORE_ENVOY)) {
                reset();
            } else {
        ...
        for (; ; ) {
            ...
            if (max == 0) {
                log.debug("Resetting, no sequences match");
                reset();
                continue;
            }
                    ...
                    .forEach(isequence ->
                    {
                        ...
                        for (int i = 0; i < byIndex.size(); i++) {
                            ...
                            if (iBookcase.getBook() == null) {
                                ...
                                byBook.put(iBookcase.getBook(), iBookcase);
                            }
                        }
                    });
            if (IntStream.range(0, certainty.length).filter(i -> certainty[i] == max).count() == 1) {
                state = SolvedState.COMPLETE;
                for (Bookcase b : getBookshelves()) {
                    boolean bookIsKnown = b.isBookSet();
                    Book setBook = b.getBook();
                    Set<Book> possible = b.getPossibleBooks();

                    if (!bookIsKnown && possible.size() == 1) {
                        setBook = possible.iterator().next();
                        bookIsKnown = true;
                    }
                    if (bookIsKnown) {
                        byBook.put(setBook, b);
                        b.setBook(setBook);
                    }
                }
            }
    ...
    private List<List<Book>> populateSequences() {
        List<List<Book>> books = Arrays.asList(
                Arrays.asList(
                        Book.KILLING_OF_A_KING,
                        Book.IDEOLOGY_OF_DARKNESS,
                        Book.RADAS_JOURNEY,
                        Book.TRANSVERGENCE_THEORY,
                        Book.TRISTESSAS_TRAGEDY,
                        Book.RADAS_CENSUS,
                        Book.TREACHERY_OF_ROYALTY,
                        Book.HOSIDIUS_LETTER,
                        Book.RICKTORS_DIARY_7,
                        Book.EATHRAM_RADA_EXTRACT,
                        Book.VARLAMORE_ENVOY,
                        Book.WINTERTODT_PARABLE,
                        Book.TWILL_ACCORD,
                        Book.BYRNES_CORONATION_SPEECH,
                        Book.SOUL_JOURNEY,
    ...
    private void add(int x, int y, int z, int i) {
        ...
        if (b == null) {
            ...
            byRoom.computeIfAbsent(Room.getRoomByWorldPoint(p), a -> new ArrayList<>()).add(b);
        }
        b.getIndex().add(i);
